...
EOF
    ...
    glm)
      key="$(resolve_key "glm" "$force_new")" || return 1
      cat > "$CONFIG_FILE" <<EOF
# Generated by cc-switch
# Provider: GLM (Zhipu AI)  Model: ${model_label}
...
EOF
      chmod 600 "$CONFIG_FILE"
      ensure_bashrc_source
      echo "✓ Switched to GLM (${model_label})."
      _cc_switch_refresh_balance
      ;;
...
EOF
    ...
    show)
      ...
      if [[ -f "$SECRETS_FILE" ]]; then
        ...
        echo "  GLM:      $(mask_value "${ZHIPU_API_KEY:-}")"
      fi
      return 0
      ;;
...
EOF
    ...
    *)
      ...
      echo "Usage: cc-switch [deepseek|minimax|glm|anthropic|show]" >&2
      return 1
      ;;
  esac
